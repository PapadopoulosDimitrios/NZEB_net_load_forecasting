...
# output all logging into a file
logging.basicConfig(filename='scenario_1.log', level=logging.INFO)
...
logger.info("GPU in use:", torch.cuda.get_device_name(torch.cuda.current_device()))
...
def ListCreatorFlagger(df, substrings=['flag', 'cos', 'sin','day_of_week', 'day_of_month', 'weekend', 'days_in_month', 'hour', 'minute']):
    ...
    if not flag_columns:
        logger.info("No columns with the specified substrings found.")
        return None, None
...
logger.info("Training set size:", len(DF_training))
logger.info("Validation set size:", len(DF_validation))
...
def FeatureSelectionWithSHAP1_fixed_test(regressor, DF_features, DF_target, ordered_features_list, test_size=672, tolerance=100):
    ...
    for i in ordered_features_list:
        ...
        #logger.info(f"Performing feature selection with features: {feature_list}")
        ...
        for train_index, test_index in splits:
            #logger.info(f"TRAIN: {train_index}, TEST: {test_index}")
            ...
            #logger.info(f"This is the error for one TS iteration: {Error}")

        # Calculate the average error across all splits
        TS_CV_error = sum(TimeSeriesCVerror) / len(TimeSeriesCVerror)
        #logger.info(f"Cumulative error of the last steps: {TS_CV_error}")
        ...
        if no_improvement_count >= tolerance:
            logger.info(f"No improvement after {tolerance} features. Stopping early.")
            break
...
logger.info(f"Best trial MSE: {best_trial.value}")
logger.info(f"Best hyperparameters: {best_trial.params}")
logger.info(f"Features used in best trial: {best_trial.user_attrs['features']}")
